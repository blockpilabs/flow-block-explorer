    ...
    if (key.startsWith('_')) {
      if (checkType(option[key]) === 'array') {
        option[key].forEach((item, index) => {
          defaultOption[key.substr(1)][index] === undefined && (defaultOption[key.substr(1)][index] = {});
          merge(defaultOption[key.substr(1)][index], item);
        });
      } else if (checkType(option[key] === 'object')) {
        merge(defaultOption[key.substr(1)], option[key]);
      } else {
