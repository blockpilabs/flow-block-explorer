...
<script>
import TitanColumn from './Column';
...
export default {
  ...
  computed: {
    ...
    calcTablePadding() {
      if (this.tablePadding === undefined) {
        return this.$titanTable.tablePadding || 0;
      }
      return this.tablePadding || 0;
    },
    calcPagerOffsetHeight() {
      return this.pagerOffset || this.$titanTable.pagerOffset;
    },
    calcCustomClassName() {
      return this.customClassName || this.$titanTable.customClassName;
    },
    calcHeightMode() {
      return this.heightMode || this.$titanTable.heightMode;
    },
    tableAttrs() {
      return {
        ...this.$attrs
  ...
  methods: {
    resize(immediate) {
      const _resizeHeight = () => {
        this.tableHeight = this.$refs.titanTableRef.clientHeight - this.calcPagerTotalHeight;
      };
      if (immediate === true) {
        _resizeHeight();
        return;
