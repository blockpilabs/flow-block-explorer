<template>
  <div
    ...
    :class="['titan-card', `is-${calcShadow}-shadow`]"
  >
    <template v-if="calcTitle">
      <div
        :class="['titan-card__header', headerClass]"
        :style="headerStyle"
      >
        <slot name="title">
          <!-- 普通标题 -->
    ...
    <div
      v-loading="loading"
      :class="['titan-card__content', contentClass]"
      :style="contentStyle"
    >
...
.titan-card.is-hover-shadow:hover, .titan-card.is-hover-shadow:focus {
  box-shadow: 0px 0px 14px 2px rgba(43, 43, 43, 0.07);
} */
...
.titan-card__header .tabs ::v-deep .el-tabs__header {
  margin-bottom: 0;
}

.titan-card__header .tabs ::v-deep .el-tabs__header .el-tabs__item {
  -webkit-touch-callout: none;
  -webkit-user-select: none;
  -khtml-user-select: none;
  -moz-user-select: none;
...
.titan-card__content .empty-place {
  width: 100%;
  height: 100%;
  display: flex;
